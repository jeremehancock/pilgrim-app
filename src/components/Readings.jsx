...
const Readings = () => {
  ...
  return (
    data ? (<Stack>
      ...
      <Typography sx={{ mb: 5 }}>{data ? formatText(addResponseToAlleluia(data.Mass_GA.text)) : ''}</Typography>
      ...
      <Typography variant='label' fontSize={11}>{data ? formatText(data.copyright.text) : ''}</Typography>
    </Stack>) : ''
  );
};
